                ...
                <div class="bg-white rounded shadow">
                    <div class="px-6 pt-6 border-bottom border-secondary-light m-3">
                        <div class="d-flex mb-6 align-items-center justify-content-between">
                            <h4 class="mb-0">List of Name Change Requests</h4>
                    ...
                    <div class="pt-4 table-responsive">
                        <table class="table mb-0 table-borderless table-striped small">
                            ...
                            <tbody>
                                ...
                                @endphp
                                @foreach ($csos as $cso)
                                    @if ($cso->nameChange->count() > 0)
                                        @foreach ($cso->nameChange as $nameChange)
                                            <tr>
                                                <td>{{ ++$count }}</td>
                                                <td>{{ $cso->english_name }}</td>
                                                <td>{{ $cso->amharic_name }}</td>
                                                <td> {{ $nameChange->send_date }}</td>
                                                <td>{{ $nameChange->status ? 'Assigned' : 'Not Assigned' }}</td>
                                                <td>
                                                    <button type="button" class="btn btn-success" data-toggle="modal"
                                                        data-target="#mdl-{{ $nameChange->id }}">Assign</button>
                                                    <div class="modal fade" id="mdl-{{ $nameChange->id }}" role="dialog">
                                                        <div class="modal-dialog">
                                                            <div class="modal-content">
                                                                ...
                                                                <form
                                                                    action="{{ route('service.name_change.assignNameChangeTask', ['id' => $nameChange->id]) }}"
                                                                    method="POST">
                                                                    @csrf
